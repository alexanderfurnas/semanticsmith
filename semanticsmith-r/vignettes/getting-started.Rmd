---
title: "Getting Started with semanticsmith"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Getting Started with semanticsmith}
...
semantic word similarity. The API is tidyverse-native: results are tibbles,
batch operations are pipe-friendly, and column references use tidy evaluation.
...
R function that takes two words (character scalars) and returns a numeric score.
...
Use any embedding matrix with row names as vocabulary. Works with output from
the `word2vec`, `text2vec`, or `wordVectors` packages:
...
- `offset` (default -0.5): Shifts cosine similarity before scaling. With -0.5,
  words need cosine > 0.5 to contribute positively to alignment.
- `scale` (default 10): Multiplier. Larger values amplify semantic differences
  relative to gap penalties.
...
and each additional position costs `gap_extend`. This means a single long
gap is cheaper than many short gaps.
...
`ssw_batch_align()` takes a data frame with text columns and adds alignment
results. It's pipe-friendly and uses tidy evaluation for column selection:
...
A header line shows the score, identity percentage, alignment length, and
gap count.

## Performance Notes

- The DP core is implemented in C (via Rcpp) for performance.
- The main bottleneck is building the similarity matrix (calling your function
  n × m times). `sim_from_embeddings()` computes cosine similarity efficiently
  with pre-computed norms.
- `ssw_batch_align()` runs sequentially over rows. For very large batches,
  consider using `furrr::future_map()` or `parallel::mclapply()` to parallelize.
